 ...
 */
function formatName(name: string): SpecialistConfig {
 ...
 */
async function extractTemplate(isSimple: boolean): Promise<string> {
    ...
    if (isSimple) {
        ...
        const types = content.match(/interface RequestAnalysis[\s\S]*?type Priority[^\n]*/)?.[0] || '';
 ...
 */
function customizeTemplate(template: string, config: SpecialistConfig): string {
    ...
    if (config.isSimple) {
        ...
        customized = customized.replace('Template simples para especialistas sem MCP', 
            `Agente especialista em ${config.name.toLowerCase()}`);
    } else {
        ...
        customized = customized.replace('Template de agente especialista com integração MCP completa',
            `Agente especialista em ${config.name.toLowerCase()} com integração MCP`);
    }
 ...
 */
async function createSpecialistFile(config: SpecialistConfig, template: string): Promise<void> {
 ...
 */
async function addImportToGuardian(config: SpecialistConfig): Promise<void> {
 ...
 */
async function addToGuardianInitialize(config: SpecialistConfig): Promise<void> {
    let content = await fs.readFile(GUARDIAN_FILE, 'utf-8');
 ...
 */
async function addToAnalyzeRequest(config: SpecialistConfig): Promise<void> {
    let content = await fs.readFile(GUARDIAN_FILE, 'utf-8');
 ...
 */
async function createBasicTest(config: SpecialistConfig): Promise<void> {
    const testContent = `/**
