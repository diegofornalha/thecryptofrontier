...
import { TaskFile, TaskOutput, AgentConfig } from '../types';
...
export abstract class BaseMCPAgent {
    ...
    try {
      ...
      for (const file of agentFiles) {
        const content = await fs.readFile(
          path.join(this.config.tasksDir, file), 
          'utf-8'
        );
   ...
   */
  async saveOutput(taskId: string, output: TaskOutput): Promise<void> {
   ...
   */
  protected extractContext(content: string): any {
  ...
  /**
   * Método abstrato para processar tarefa
   * Deve ser implementado por cada agente
   */
  abstract processTask(task: TaskFile): Promise<TaskOutput>;
