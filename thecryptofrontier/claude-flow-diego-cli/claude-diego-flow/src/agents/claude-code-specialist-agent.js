...
export class ClaudeCodeSpecialistAgent extends Agent {
    constructor() {
        ...
        this.version = '4.1.0'; // Updated with model differences, citations, search guidelines
    ...
    async analyzeSecurityPractices() {
    ...
    async generalClaudeCodeAnalysis() {
    ...
    async analyzeArtifacts() {
        ...
        if ((_a = this.knowledgeBase.artifactsAdvanced) === null || _a === void 0 ? void 0 : _a.designPrinciples) {
        ...
        report += '\n## ⚠️ RESTRIÇÃO CRÍTICA\n';
        if ((_b = this.knowledgeBase.artifactsAdvanced) === null || _b === void 0 ? void 0 : _b.criticalRestriction) {
    ...
    async analyzeVerbosityGuidelines() {
    ...
    analyzeClaudeMdContent(content) {
        let analysis = '';
...
`;
    ...
    generateConventionRecommendations() {
        return `
...
`;
    ...
    async analyzeProjectProductivity() {
        let analysis = '';
...
`;
    ...
    getModePractices() {
        ...
        for (const [modeName, mode] of Object.entries(this.knowledgeBase.modes)) {
            practices += `\n### Modo ${this.formatModeName(modeName)}\n`;
    ...
    formatCapability(capability) {
        return capability.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    }
    async shutdown() {
    ...
    detectCategory(improvement) {
        ...
        if (lower.includes('security') || lower.includes('segurança'))
            return 'security';
        if (lower.includes('tool') || lower.includes('ferramenta'))
            return 'toolUsage';
        if (lower.includes('code') || lower.includes('código'))
            return 'codeConventions';
        if (lower.includes('task') || lower.includes('tarefa'))
            return 'taskManagement';
        if (lower.includes('communication') || lower.includes('comunicação'))
            return 'communication';
        return null;
    }
    updateCommands(improvement) {
        // Lógica para extrair e adicionar novos comandos
        // Esta é uma implementação simplificada
        console.log('📝 Novo comando aprendido');
    }
    improvePatternDetection(improvement) {
