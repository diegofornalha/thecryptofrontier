...
program
    .name('claude-diego-flow-diego')
    .description('Sistema de orquestração de agentes Claude-Diego com DiegoTools MCP')
    .version('1.0.0');
...
agentCmd
    ...
    try {
        ...
        // Buscar template
        const templateConfig = mcpAgentTemplates[template];
        if (!templateConfig) {
...
memoryCmd
    ...
    try {
        const mcpBridge = new MCPBridge();
        await mcpBridge.connect();
        const results = await mcpBridge.searchMemory(options.user, query, parseInt(options.limit));
...
memoryCmd
    ...
    try {
        ...
        const result = await mcpBridge.addMemory(options.user, content, { ...metadata, category: options.category });
