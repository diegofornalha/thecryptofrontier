...
export class GitMemoryServer {
    ...
    setupRoutes() {
        ...
        this.app.post('/api/decisions', async (req, res) => {
            try {
                ...
                await this.gitMemory.recordAgentDecision(agentName, reasoning, expectedImpact || 'TBD', filesAffected || []);
                res.json({
                    success: true,
        ...
        this.app.get('/api/search', async (req, res) => {
            try {
                ...
                const timeline = await this.gitMemory.createGitMemoryTimeline(parseInt(days));
                const results = this.searchTimeline(timeline, query);
                res.json({
                    success: true,
                    query,
                    results,
     ...
     */
    formatTimelineForFrontend(timeline) {
        const events = [];
