...
async function coordinateUnification() {
    ...
    try {
        ...
        await guardian.logTaskProgress('analyze-1', `Encontrados ${filesFound.filter(f => f.includes('image_generation')).length} arquivos de geração de imagem`);
        await guardian.logTaskProgress('analyze-2', `Encontrados ${filesFound.filter(f => f.includes('algolia')).length} arquivos Algolia`);
        ...
        await guardian.logTaskError('analyze-1', error);
    }
}
// Executar
