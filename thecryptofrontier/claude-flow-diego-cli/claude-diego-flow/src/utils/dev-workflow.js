...
async function runDevWorkflow() {
    ...
    try {
        ...
        const gitStatusTask = orchestrator.createTask('Verificar status do repositório Git', {
            assignTo: gitManager.id,
            metadata: { detailed: true }
        });
        // Task 2: Criar issue no GitHub
        const issueTask = orchestrator.createTask('Criar issue sobre integração MCP', {
