...
export class GitMemoryIntegration {
     ...
     */
    async createCommitMemorySnapshot(commitMessage) {
        try {
     ...
     */
    async recordAgentDecision(agentName, reasoning, expectedImpact, filesAffected = []) {
        try {
     ...
     */
    async createGitMemoryTimeline(days = 7) {
        try {
            ...
            for (const commit of commits) {
                ...
                const agentDecisions = await this.memory.searchMemories(`commit ${commit.shortHash} decisão agente`, 10);
                ...
                const timelineEntry = {
                    ...
                    agentDecisions: agentDecisions.map(decision => {
                        ...
                        return ({
                            ...
                            agentName: ((_a = decision.metadata) === null || _a === void 0 ? void 0 : _a.agent_name) || 'Unknown',
                            decisionTimestamp: ((_b = decision.metadata) === null || _b === void 0 ? void 0 : _b.decision_timestamp) || commit.date,
                            preCommitState: {
                                memorySnapshot: [],
                                activeDecisions: []
                            },
                            reasoning: decision.content,
                            filesAffected: ((_c = decision.metadata) === null || _c === void 0 ? void 0 : _c.files_affected) || [],
                            expectedImpact: ((_d = decision.metadata) === null || _d === void 0 ? void 0 : _d.expected_impact) || 'N/A'
                        });
                    }),
                    memoryBackup,
...
`;
     ...
     */
    async getFileDecisionHistory(filePath) {
        const decisions = await this.memory.searchMemories(`arquivo ${filePath} decisão modificação`, 20);
        return decisions.map(decision => {
            var _a, _b, _c, _d;
            return ({
                timestamp: (_a = decision.metadata) === null || _a === void 0 ? void 0 : _a.decision_timestamp,
                commit: (_b = decision.metadata) === null || _b === void 0 ? void 0 : _b.commit_short,
                agent: (_c = decision.metadata) === null || _c === void 0 ? void 0 : _c.agent_name,
                reasoning: decision.content,
                impact: (_d = decision.metadata) === null || _d === void 0 ? void 0 : _d.expected_impact
            });
        });
    }
    /**
    ...
    async getGitContext() {
        ...
        try {
            ...
            const additions = ((_a = stats.match(/(\d+) insertion/)) === null || _a === void 0 ? void 0 : _a[1]) ? parseInt(stats.match(/(\d+) insertion/)[1]) : 0;
            const deletions = ((_b = stats.match(/(\d+) deletion/)) === null || _b === void 0 ? void 0 : _b[1]) ? parseInt(stats.match(/(\d+) deletion/)[1]) : 0;
            return {
                hash,
                shortHash,
                message,
    ...
    async getRecentCommits(days) {
        try {
            ...
            const output = execSync(`git log --since="${since}" --pretty=format:"%H|%h|%s|%an|%ai" --name-only`, { encoding: 'utf8' });
    ...
    async findMemoryBackupForCommit(commitHash) {
        try {
    ...
    generateTimelineSummary(commit, decisions) {
        if (decisions.length === 0) {
            return `Commit ${commit.shortHash}: ${commit.message} (sem decisões de agentes registradas)`;
        }
        const agentCount = new Set(decisions.map(d => { var _a; return (_a = d.metadata) === null || _a === void 0 ? void 0 : _a.agent_name; })).size;
     ...
     */
    async recordCommitCompletion(commitHash, commitMessage) {
        await this.memory.addMemory({
