...
interface ConversationalMessage {
  role: 'system' | 'agent' | 'user' | 'result' | 'action';
  content: string;
  timestamp?: string;
}

interface Mem0OSSRequest {
...
export class Mem0MCPBridge {
  ...
  private mem0OSSUrl: string;
  private port: number;

  constructor(chromaUrl: string = 'http://localhost:8000', port: number = 3002) {
    this.app = express();
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/mcp/search_memory', async (req, res) => {
      try {
        ...
        const mcpResults = response.data.memories?.map((memory: any) => ({
          id: memory.id,
    ...
    this.app.get('/mcp/list_memories/:user_id', async (req, res) => {
      try {
        ...
        const mcpMemories = response.data.memories?.map((memory: any) => ({
          id: memory.id,
          content: memory.memory || memory.content,
          created_at: memory.created_at,
          metadata: memory.metadata
    ...
    this.app.post('/oss/add_conversation', async (req, res) => {
      try {
        ...
        const ossRequest: Mem0OSSRequest = {
          messages,
          user_id,
          metadata: {
            ...metadata,
