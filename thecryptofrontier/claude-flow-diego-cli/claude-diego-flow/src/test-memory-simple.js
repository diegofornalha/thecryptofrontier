...
async function testMemoryAgent() {
    ...
    try {
        ...
        // Executar busca
        const results = await memory.search('organização projetos estrutura', { user_id: 'AgentLog', limit: 10 });
    ...
    try {
        ...
        await logAgent.start('Memory Enhanced Agent', AgentType.CUSTOM, addTaskId, 'Adicionar memória sobre execução de teste', {
            taskType: 'memory-add',
            complexity: 'simple'
        });
        const startTime = Date.now();
        // Adicionar memória
        await memory.add('Teste do Memory Enhanced Agent executado com sucesso. O AgentLog está registrando métricas de busca e consolidação.', { user_id: 'TestAgent' });
        const duration = Date.now() - startTime;
        console.log(`✅ Memória adicionada em ${duration}ms`);
        await logAgent.end('Memory Enhanced Agent', addTaskId, 'completed', undefined, {
            duration,
        ...
        await logAgent.end('Memory Enhanced Agent', addTaskId, 'error', error.message);
    }
