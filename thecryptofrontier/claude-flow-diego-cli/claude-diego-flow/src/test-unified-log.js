...
async function testUnifiedLog() {
    ...
    await logAgent.start('Test Agent', AgentType.IMPLEMENTER, taskId1, 'Implementar funcionalidade de teste', { taskType: 'implementation', complexity: 'medium' });
    ...
    await logAgent.start('Error Test Agent', AgentType.ANALYST, taskId2, 'Tarefa que vai falhar', { taskType: 'validation', complexity: 'simple' });
    await new Promise(resolve => setTimeout(resolve, 500));
    await logAgent.end('Error Test Agent', taskId2, 'error', 'Arquivo não encontrado: config.json', { attemptedPath: '/path/to/config.json' });
