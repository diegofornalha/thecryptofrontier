...
export class MemoryService {
  ...
  async searchMemories(input: SearchMemoryInput): Promise<Memory[]> {
    try {
      // Check cache first
  ...
  async updateMemory(memoryId: string, userId: string, input: UpdateMemoryInput): Promise<Memory | null> {
    try {
      const memory = await Memory.findOne({
        where: { id: memoryId, userId }
      });
