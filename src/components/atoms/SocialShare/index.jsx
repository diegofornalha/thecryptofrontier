...
export default function SocialShare(props) {
    ...
    return (<div className={classNames('flex flex-wrap items-center', className)} {...(enableAnnotations && { 'data-sb-field-path': 'socialShare' })}>
            <ul className="flex flex-wrap items-center justify-center w-full">
                {socialLinks.map((link, index) => (<li key={index} className="m-2">
                        <Social {...link} className="text-2xl bg-white hover:bg-gray-200 p-3 rounded-full shadow-md transform transition-transform duration-200 hover:scale-110" {...(enableAnnotations && { 'data-sb-field-path': `.${index}` })}/>
                    </li>))}
            </ul>
        </div>);
}
