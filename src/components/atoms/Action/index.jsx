...
export default function Action(props) {
    ...
    const annotations = fieldPath
        ...
        : {};
    const type = (_a = props.__metadata) === null || _a === void 0 ? void 0 : _a.modelName;
    ...
    const renderIcon = () => {
        if (!showIcon || !IconComponent)
            return null;
        return (<IconComponent className={cn("shrink-0 size-[1.25em]", {
    ...
    if (url) {
        return (<Button variant={variant} className={className} asChild id={elementId} aria-label={altText} {...annotations}>
                <NextLink href={url}>
                    {buttonContent}
                </NextLink>
            </Button>);
    ...
    return (<Button variant={variant} className={className} id={elementId} aria-label={altText} {...annotations}>
            {buttonContent}
        </Button>);
}
