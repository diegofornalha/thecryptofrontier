...
export default function StrapiConnectionTest() {
  ...
  const [postsStatus, setPostsStatus] = useState<TestResult | null>(null);
  const [authorsStatus, setAuthorsStatus] = useState<TestResult | null>(null);
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setPostsStatus({
        success: true,
        message: `${postsResponse.data?.length || 0} posts encontrados`,
        data: adaptedPosts
      });
    } catch (error) {
      setPostsStatus({
    ...
    try {
      ...
      setAuthorsStatus({
        success: true,
        message: `${authorsResponse.data?.length || 0} autores encontrados`,
  ...
  const StatusCard = ({ title, status }: { title: string; status: TestResult | null }) => {
    if (!status) return null;
    
    return (
      <div className={`p-4 rounded-lg border ${status.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
