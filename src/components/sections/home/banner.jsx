...
export default function Banner({ title, category = 'THE CRYPTO BASIC', imageUrl, slug, subtitle, showBitcoin = true, showRocket = true }) {
    const content = (<article className={`relative w-full h-full bg-gray-900 rounded-xl overflow-hidden ${slug ? 'cursor-pointer group' : ''}`}>
      <div className="relative h-[400px] sm:h-[450px] md:h-[500px] lg:h-[550px] xl:h-[600px] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          {imageUrl ? (<Image src={imageUrl} alt={title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" priority/>) : (<div className="relative w-full h-full">
              ...
              {showBitcoin && (<div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              ...
              {showRocket && (<div className="absolute bottom-[15%] sm:bottom-[20%] left-[5%] sm:left-[10%] transform rotate-45 animate-float">
        ...
        <div className="relative z-20 w-full h-full flex flex-col justify-end p-6 sm:p-8 md:p-10 lg:p-12">
          ...
          <div className="space-y-2 sm:space-y-3 md:space-y-4">
            ...
            {subtitle && (<p className="text-gray-200 text-sm sm:text-base md:text-lg lg:text-xl max-w-3xl">{subtitle}</p>)}
          </div>
        </div>
      </div>
    </article>);
    return slug ? (<Link href={`/post/${slug}`}>
      {content}
    </Link>) : (content);
}
