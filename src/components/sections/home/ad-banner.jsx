...
export default function AdBanner({ title = 'Sinais Cripto Expert', subtitle = 'Lucre de R$ 500,00 a R$ 5.000 em média por dia no criptomercado, sem precisar olhar gráficos, notícias, nem fazer cursos enormes.', link = 'https://eternityscale.com.br/sce-blog/', targetBlank = true, className = '', showBitcoinAnimation = true }) {
    const content = (<div className={`relative w-full h-[250px] sm:h-[280px] md:h-[300px] lg:h-[320px] xl:h-[350px] rounded-xl overflow-hidden cursor-pointer group ${className}`}>
      ...
      <div className="absolute inset-0">
        ...
        {showBitcoinAnimation && (<div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-40 group-hover:opacity-60 transition-opacity duration-500">
        ...
        {/* Foguete - Responsivo */}
        {showBitcoinAnimation && (<div className="absolute bottom-[15%] sm:bottom-[20%] left-[5%] sm:left-[10%] transform rotate-45 animate-float opacity-50 group-hover:opacity-70 transition-opacity duration-500">
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 z-10">
        <div className="space-y-2 sm:space-y-3 md:space-y-4">
          {title && (<h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-bold text-white leading-tight shadow-lg drop-shadow-2xl">
              {title}
            </h2>)}
          {subtitle && (<p className="text-gray-200 text-sm sm:text-base md:text-lg lg:text-xl max-w-3xl drop-shadow-lg">
              {subtitle}
            </p>)}
        </div>
      </div>
    ...
    if (link) {
        if (targetBlank || link.startsWith('http')) {
            return (<a href={link} target={targetBlank ? "_blank" : "_self"} rel={targetBlank ? "noopener noreferrer" : undefined} className="block">
          {content}
        </a>);
        }
        else {
            return (<Link href={link} className="block">
          {content}
        </Link>);
        }
    }
