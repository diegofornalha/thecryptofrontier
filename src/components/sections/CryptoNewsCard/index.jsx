...
export default function CryptoNewsCard({ title, slug, excerpt, coverImage, featuredImageUrl, authorName, publishedAt, category, readTime = 3, featured = false, horizontal = false, }) {
    var _a;
    ...
    if (featuredImageUrl) {
        ...
        try {
            ...
            imageUrl = ((_a = urlForImage(coverImage)) === null || _a === void 0 ? void 0 : _a.url()) || '/placeholder-news.jpg';
        }
        catch (error) {
    ...
    if (horizontal) {
        return (<article className="group flex gap-4 pb-6 border-b border-gray-200 hover:opacity-80 transition-opacity">
        {/* Image */}
        <Link href={`/post/${slug}`} className="flex-shrink-0">
          <div className="relative w-[140px] h-[105px] overflow-hidden rounded">
            <Image src={imageUrl} alt={title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" placeholder="empty" style={{ backgroundColor: '#f0f0f0' }}/>
          </div>
        </Link>
        ...
        <div className="flex-1 min-w-0">
          {/* Category */}
          {category && category.title && (<Link href={`/categoria/${category.slug}`} className="inline-block text-xs font-bold text-[#4db2ec] uppercase tracking-wide mb-2 hover:underline">
              {category.title}
            </Link>)}

          {/* Title */}
          <h3 className="mb-2">
            <Link href={`/post/${slug}`} className="text-lg font-bold text-[#111] leading-tight hover:text-[#4db2ec] transition-colors line-clamp-2">
              {title}
            </Link>
          </h3>
    ...
    return (<article className={`group ${featured ? 'col-span-2 row-span-2' : ''}`}>
      <Link href={`/post/${slug}`} className="block">
        ...
        <div className={`relative overflow-hidden rounded mb-3 ${featured ? 'h-[400px]' : 'h-[200px]'}`}>
          ...
          {/* Category Badge */}
          {category && category.title && (<div className="absolute top-3 left-3">
        ...
        <div>
          ...
          {featured && excerpt && (<p className="text-[#666] text-sm mb-3 line-clamp-2">
              {excerpt}
            </p>)}
