...
export default function ImageGallerySection(props) {
    ...
    return (<Section elementId={elementId} className="sb-component-image-gallery-section" colors={colors} backgroundImage={backgroundImage} styles={styles === null || styles === void 0 ? void 0 : styles.self} {...getDataAttrs(props)}>
            <div className={classNames('w-full', 'flex', 'flex-col', mapStyles({ alignItems: (_b = (_a = styles === null || styles === void 0 ? void 0 : styles.self) === null || _a === void 0 ? void 0 : _a.justifyContent) !== null && _b !== void 0 ? _b : 'flex-start' }))}>
                ...
                <ImageGalleryVariants motion={motion} images={images} hasTopMargin={!!((badge === null || badge === void 0 ? void 0 : badge.label) || (title === null || title === void 0 ? void 0 : title.text) || subtitle)} justifyContent={(_c = styles === null || styles === void 0 ? void 0 : styles.self) === null || _c === void 0 ? void 0 : _c.justifyContent} hasAnnotations={enableAnnotations}/>
            </div>
        </Section>);
}
...
function ImageGalleryStaticGrid({ images = [], hasTopMargin, justifyContent = 'flex-start', hasAnnotations }) {
    ...
    return (<div className={classNames('w-full', 'flex', 'flex-wrap', 'items-center', mapStyles({ justifyContent: justifyContent }), { 'mt-12': hasTopMargin })} {...(hasAnnotations && { 'data-sb-field-path': '.images' })}>
            {images.map((image, index) => (<ImageBlock key={index} {...image} {...(hasAnnotations ? { 'data-sb-field-path': `.${index}` } : {})}/>))}
        </div>);
}
function ImageGalleryAnimatedGrid({ images = [], motion, hasTopMargin, hasAnnotations }) {
    ...
    return (<div className="w-full">
            <div className={classNames('sb-image-strip-wrap', 'relative', 'before:absolute', 'before:h-full', 'before:w-1/6', 'before:left-0', 'before:top-0', 'before:content-[""]', 'before:bg-gradient-to-l', 'before:z-10', 'after:absolute', 'after:h-full', 'after:w-1/6', 'after:right-0', 'after:top-0', 'after:content-[""]', 'after:bg-gradient-to-r', 'after:z-10', { 'mt-12': hasTopMargin })}>
                <div className={classNames('sb-image-strip', 'overflow-hidden', 'whitespace-nowrap', 'relative')} style={imageGridStyle}>
                    <div className={classNames('sb-image-strip-track', 'flex', ' items-center', 'w-max', motion === 'move-to-left' ? 'sb-animate-slide-left' : 'sb-animate-slide-right')}>
