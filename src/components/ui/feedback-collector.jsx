...
export function FeedbackCollector({ componentName, variant = 'default', className }) {
    ...
    return (<Dialog onOpenChange={(open) => !open && handleReset()}>
      <DialogTrigger asChild>
        {variant === 'minimal' ? (<Button variant="ghost" size="sm" className={className}>
            Feedback
          </Button>) : (<Button variant="outline" size="sm" className={className}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        {submitted ? (<div className="py-6 text-center">
            ...
            <div className="space-y-4 py-4">
              <div>
                ...
                <div className="flex justify-center space-x-2">
                  {[1, 2, 3, 4, 5].map((value) => (<Button key={value} type="button" variant={rating === value ? "default" : "outline"} size="sm" onClick={() => setRating(value)}>
                      {value}
                    </Button>))}
                </div>
              ...
              <div>
                ...
                <Textarea placeholder="Como podemos melhorar este componente?" value={feedback} onChange={(e) => setFeedback(e.target.value)} className="resize-none" rows={4}/>
              </div>
            </div>
