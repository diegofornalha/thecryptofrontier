...
export default function ThreeColumnLayout({ children, sidebar, title, breadcrumbs = [], }) {
          ...
          <div className="max-w-7xl mx-auto px-4">
            <nav className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              {breadcrumbs.map((crumb, index) => (<React.Fragment key={index}>
                  ...
                  {crumb.href ? (<a href={crumb.href} className="hover:text-[#4db2ec] transition-colors">
                      {crumb.label}
                    </a>) : (<span className="text-gray-900">{crumb.label}</span>)}
                </React.Fragment>))}
            </nav>
          </div>
        </div>
