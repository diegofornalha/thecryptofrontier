...
export default function PostFeedLayout(props) {
    ...
    return (<BaseLayout page={page} site={site}>
            <main id="main" className="sb-layout sb-page-layout">
                {title && (<h1 className="sr-only" {...(enableAnnotations && { 'data-sb-field-path': 'title' })}>
                        {title}
                    </h1>)}
                {renderSections(topSections, 'topSections', enableAnnotations)}
                <PostFeedSection {...postFeed} posts={items} pageLinks={pageLinks} searchBox={searchBox} enableAnnotations={enableAnnotations} {...(enableAnnotations && { 'data-sb-field-path': 'postFeed' })}/>
...
function renderSections(sections, fieldName, enableAnnotations) {
    ...
    return (<div {...(enableAnnotations && { 'data-sb-field-path': fieldName })}>
            ...
            return (<Component key={index} {...section} enableAnnotations={enableAnnotations} {...(enableAnnotations && { 'data-sb-field-path': `${fieldName}.${index}` })}/>);
        })}
        </div>);
}
function SearchBox({ enableSearch }) {
    ...
    return (<div className="w-full mb-9" style={searchBoxStyle}>
            <AutoCompletePosts />
        </div>);
}
function PageLinks({ pageIndex, baseUrlPath, numOfPages }) {
    ...
    if (pageIndex > 0) {
        pageLinks.push(<PageLink key="prev" pageIndex={pageIndex - 1} baseUrlPath={baseUrlPath}>
                <ChevronLeftIcon className="w-6 h-6 fill-current"/>
            </PageLink>);
    }
    else {
        pageLinks.push(<PageLinkDisabled key="prev">
                <ChevronLeftIcon className="w-6 h-6 fill-current"/>
            </PageLinkDisabled>);
    }
    // if startIndex is not 0, then render the first page followed by ellipsis, if needed.
    if (startIndex > 0) {
        pageLinks.push(<PageLink key="0" pageIndex={0} baseUrlPath={baseUrlPath}>
                1
            </PageLink>);
        if (startIndex > 1) {
    ...
    for (let i = startIndex; i <= endIndex; i++) {
        if (pageIndex === i) {
            ...
            pageLinks.push(<PageLink key={i} pageIndex={i} baseUrlPath={baseUrlPath}>
                    {i + 1}
                </PageLink>);
        }
    }
    ...
    if (endIndex < numOfPages - 1) {
        ...
        pageLinks.push(<PageLink key={numOfPages - 1} pageIndex={numOfPages - 1} baseUrlPath={baseUrlPath}>
                {numOfPages}
            </PageLink>);
    }
    // renders next "→" button, if the current page is the last page, the button is disabled
    if (pageIndex < numOfPages - 1) {
        pageLinks.push(<PageLink key="next" pageIndex={pageIndex + 1} baseUrlPath={baseUrlPath}>
                <ChevronRightIcon className="w-6 h-6 fill-current"/>
            </PageLink>);
    }
    else {
        pageLinks.push(<PageLinkDisabled key="next">
                <ChevronRightIcon className="w-6 h-6 fill-current"/>
            </PageLinkDisabled>);
    }
...
function PageLink({ pageIndex, baseUrlPath, children }) {
    return (<Link href={urlPathForPageAtIndex(pageIndex, baseUrlPath)} className="w-10 h-10 p-0 text-sm sb-component-button sb-component-button-secondary shrink-0">
            {children}
        </Link>);
}
function PageLinkDisabled({ children }) {
    return (<span key="next" className="w-10 h-10 p-0 text-sm opacity-25 pointer-events-none sb-component-button sb-component-button-secondary shrink-0">
            {children}
        </span>);
}
function Ellipsis() {
