...
export default function PostLayout(props) {
    ...
    return (<BaseLayout page={page} site={site}>
            <main id="main" className="sb-layout sb-post-layout">
                <article className="px-4 py-16 sm:py-28">
                    <div className="mx-auto max-w-screen-2xl">
                        <header className="max-w-4xl mx-auto mb-12 text-center">
                            ...
                            {(showDate || (showAuthor && author)) && (<div className="mt-4 text-sm uppercase">
                                    {showDate && (<time dateTime={dateTimeAttr} {...(enableAnnotations && { 'data-sb-field-path': 'date' })}>
                                            {formattedDate}
                                        </time>)}
                                    {showAuthor && author && showDate && <span className="mx-2">|</span>}
                                    {showAuthor && author && (<PostAuthor author={author} enableAnnotations={enableAnnotations}/>)}
                                </div>)}
                        </header>
                        {markdown_content && (<Markdown options={{ forceBlock: true }} className="max-w-3xl mx-auto sb-markdown" {...(enableAnnotations && { 'data-sb-field-path': 'markdown_content' })}>
                                {markdown_content}
                            </Markdown>)}
                    </div>
                </article>
                ...
                {categories && categories.length > 0 && (<RelatedPostsSection title="Posts Relacionados" colors={page.colors || 'bg-light-fg-dark'} currentPostCategories={categories} currentPostSlug={slug} limit={3}/>)}
                
                {bottomSections.length > 0 && (<div {...(enableAnnotations && { 'data-sb-field-path': 'bottomSections' })}>
                ...
                return (<Component key={index} {...section} enableAnnotations={enableAnnotations} {...(enableAnnotations && { 'data-sb-field-path': `bottomSections.${index}` })}/>);
            })}
                    </div>)}
                ...
                <div className="mx-auto max-w-screen-2xl px-4 pb-16">
                    <div className="max-w-3xl mx-auto mt-12 border-t border-b py-6 bg-gray-50 rounded-lg shadow-sm">
                        ...
                        <SocialShare title={title} url={postUrl} enableAnnotations={enableAnnotations} className="justify-center"/>
                    </div>
                </div>
            </main>
        </BaseLayout>);
...
function PostAuthor({ author, enableAnnotations }) {
    ...
    return author.slug ? (<Link {...(enableAnnotations && { 'data-sb-field-path': 'author' })} href={`/mcpx/author/${author.slug}`}>
            {authorName}
        </Link>) : (<span {...(enableAnnotations && { 'data-sb-field-path': 'author' })}>{authorName}</span>);
}
/*
