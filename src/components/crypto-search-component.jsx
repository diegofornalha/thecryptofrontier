...
const Hit = ({ hit }) => {
    ...
    return (<article className="border-b border-gray-200 pb-6 mb-6 last:border-0">
      {postUrl ? (<Link href={postUrl}>
          <div className="flex gap-6 hover:opacity-80 transition-opacity">
            {/* Imagem */}
            {hit.featuredImage && (<div className="flex-shrink-0">
                <div className="relative w-[240px] h-[135px] overflow-hidden rounded">
                  <img src={hit.featuredImage} alt={hit.title} className="w-full h-full object-cover"/>
                </div>
              </div>)}
            ...
            <div className="flex-1">
              {/* Categoria */}
              {hit.categories && hit.categories.length > 0 && (<div className="mb-2">
              ...
              <div className="flex items-center gap-4 text-xs text-[#999]">
                {hit.date && (<span suppressHydrationWarning>
                    {formatDate(hit.date)}
                  </span>)}
                {hit.timeToRead && (<span>{hit.timeToRead} min de leitura</span>)}
              </div>
            </div>
          </div>
        </Link>) : (<div className="opacity-50">
...
const CryptoSearchComponent = () => {
    ...
    return (<div className="w-full">
      <InstantSearch searchClient={searchClient} indexName={indexName} searchState={searchState} onSearchStateChange={setSearchState}>
        {/* Campo de busca */}
        <div className="mb-8">
