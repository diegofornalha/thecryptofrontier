...
export const Featured = {
    ...
    decorators: [
        (Story) => (<div className="max-w-[600px]">
        <Story />
      </div>),
    ],
    parameters: {
...
export const Grid = {
    render: () => (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <CryptoNewsCard title="Solana registra crescimento de 300% em TVL" slug="solana-crescimento-tvl" coverImage={mockImage} author={{ firstName: 'Pedro', lastName: 'Costa' }} publishedAt={new Date().toISOString()} category={{ title: 'Altcoins', slug: 'altcoins' }} readTime={4}/>
      <CryptoNewsCard title="SEC aprova primeiro ETF de Bitcoin spot nos EUA" slug="sec-aprova-etf-bitcoin" coverImage={mockImage} author={{ firstName: 'Ana', lastName: 'Lima' }} publishedAt={new Date().toISOString()} category={{ title: 'Regulação', slug: 'regulacao' }} readTime={6}/>
      <CryptoNewsCard title="NFTs voltam a ganhar força com novos projetos" slug="nfts-novos-projetos" coverImage={mockImage} author={{ firstName: 'Lucas', lastName: 'Oliveira' }} publishedAt={new Date().toISOString()} category={{ title: 'NFTs', slug: 'nfts' }} readTime={5}/>
    </div>),
    parameters: {
        docs: {
            description: {
...
export const ListaHorizontal = {
    render: () => (<div className="max-w-[400px] space-y-4">
      <CryptoNewsCard horizontal title="Binance lança novo programa de staking" slug="binance-programa-staking" coverImage={mockImage} author={{ firstName: 'Carlos', lastName: 'Mendes' }} publishedAt={new Date().toISOString()} category={{ title: 'Exchanges', slug: 'exchanges' }} readTime={3}/>
      <CryptoNewsCard horizontal title="Cardano anuncia parceria com governo africano" slug="cardano-parceria-africa" coverImage={mockImage} author={{ firstName: 'Julia', lastName: 'Ferreira' }} publishedAt={new Date().toISOString()} category={{ title: 'Blockchain', slug: 'blockchain' }} readTime={4}/>
      <CryptoNewsCard horizontal title="MicroStrategy compra mais 5.000 bitcoins" slug="microstrategy-compra-bitcoins" coverImage={mockImage} author={{ firstName: 'Roberto', lastName: 'Silva' }} publishedAt={new Date().toISOString()} category={{ title: 'Mercado', slug: 'mercado' }} readTime={2}/>
    </div>),
    parameters: {
        docs: {
            description: {
