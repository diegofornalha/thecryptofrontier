...
check_redis() {
    ...
    if docker ps | grep -q "redis-crewai-cache"; then
        ...
        if python test_redis_connection.py > /dev/null; then
...
start_processor() {
    ...
    if [ -f "docker-compose.yml" ]; then
        ...
        $DOCKER_COMPOSE up -d queue-processor
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ -f "process_article_queue.py" ]; then
                nohup python process_article_queue.py --loop 30 > logs/processor.log 2>&1 &
                PROCESSOR_PID=$!
